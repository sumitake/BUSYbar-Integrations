...
The Nyan Cat animation is compiled into a native `.anim` asset and uploaded to the device on the integration's first run. Once stored, the device plays it entirely in firmware — no further communication with the host beyond the periodic poll interval. This design keeps host overhead minimal: the integration spends most cycles simply checking whether to show the filler or not (when higher-priority content is active), not streaming animation frames.
...
If you modify the source or rebuild the animation, the agent automatically re-uploads the asset to the device on its next startup.
