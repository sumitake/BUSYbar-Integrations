...
The integration looks ahead 12 hours by default and draws at priority 20 on the display. If an active BUSY session exists on the device, the calendar event display is suppressed in favor of the busy state (priority 90). If the `ci_status` integration is also running with `show_running` enabled, expect the calendar to lose the screen for roughly half of any period where a CI run is active — see `ci_status`'s README ("Running-Job Badge") for why; it's a firmware limitation (no restore-after-occlusion), not a bug in either integration.
